...
class KGEnricher:
    ...
    def _monarch_neighbors(self, curie: str) -> list:
        try:
            url = f"{self.monarch_url}association/from/{curie}"
            resp = requests.get(url, params={"rows": 20, "fetch_objects": True}, timeout=15)
            ...
            for assoc in associations:
                ...
                label = obj.get("label", "")
                score = assoc.get("score", 1.0)
                if label:
                    results.append({"label": label, "curie": obj.get("id", ""), "score": score})
            return results
